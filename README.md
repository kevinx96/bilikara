...
- 本地缓存 / 在线外挂播放器模式
- 本地播放模式支持音画延迟补偿，可按毫秒调节音频提前 / 延后
- 本地播放模式支持独立音量控制、静音，以及音量 / 延迟本地记忆与自动恢复
- 默认加到列表末尾 / 顶歌到下一首
- 切歌、移除、拖动、顶歌到下一首
- 多分 p 视频自动判断有效分 p，自动缓存多音轨，可随时切换；切换时会同步当前播放进度与播放状态
- 同一局域网内手机端控制台，可远程暂停 / 播放、前后跳转、切换音轨、调节音量和音画延迟
- 加入列表后自动后台缓存，缓存失败 / 长时间无变化显示重试按钮
- 列表中展示缓存状态和完成标记
...
- 如果当前歌曲已经缓存完成，切到“本地播放模式”后会使用浏览器里的分离视频 / 音频播放器播放本地文件
- 本地播放模式下，视频与音频流会分开同步，用来支持独立的音画延迟补偿、音量控制和静音
